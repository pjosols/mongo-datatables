"""Tests for DT_RowClass, DT_RowData, DT_RowAttr support in Editor responses."""
import pytest
...
def _make_editor(request_args, row_class=None, row_data=None, row_attr=None):
    mongo = MagicMock()
    doc_id = str(ObjectId())
...
def _mock_doc(editor):
    """Return a fake MongoDB document with an ObjectId."""
    oid = ObjectId()
...
class TestEditorRowMetadataAbsent:
    def test_no_row_class_key_absent(self):
        editor = _make_editor({"action": "create", "data": {"0": {"name": "Alice"}}})
        _mock_doc(editor)
