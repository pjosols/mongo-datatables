...
def _coerce_int(value: Any, name: str, default: int, minimum: int | None = None) -> int:
    ...
    Raises InvalidDataError if value cannot be converted. Returns coerced int.
    """
    if value is None:
        return default
    try:
...
def _validate_search_dict(search: Any, context: str) -> None:
    ...
    Raises InvalidDataError if search is not a dict or missing required keys ('value', 'regex').
    """
...
def _validate_columns(columns: Any) -> None:
    ...
    Raises InvalidDataError if columns is not a list, contains non-dict entries, missing required keys ('searchable', 'orderable', 'search'), invalid field names, or malformed search dicts.
    """
...
def _validate_order(order: Any, num_columns: int) -> None:
    ...
    Raises InvalidDataError if order is not a list, contains non-dict entries, missing required keys ('column', 'dir'), invalid column indices, or invalid sort directions ('asc', 'desc').
    """
...
def _normalize_request_args(request_args: Dict[str, Any]) -> None:
    """Fill defaults for missing optional keys and normalize sub-dicts in-place.

    Provides defaults for start/length/search/columns/order when absent,
    and normalises incomplete column/search sub-dicts so strict validators
...
def validate_request_args(request_args: Any) -> Dict[str, Any]:
    ...
    Checks for required top-level keys (draw, columns, order, search), validates types of sub-structures,
    and sanitizes numeric parameters (draw, start, length). Missing optional keys are filled with defaults before validation.

    request_args: raw request parameters from the DataTables Ajax call.
    Raises InvalidDataError if validation fails. Returns the validated dict (same object, numeric fields coerced).
    """
